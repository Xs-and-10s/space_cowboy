...
`datastar_beam` does not need to know which template engine produced your
...
2. Pass that rendered iodata, binary, or string to `datastar_beam`.
...
- `datastar_beam:patch_elements/1,2` accepts rendered fragments as iodata,
  binaries, or strings.
- `datastar_beam` does not take dependencies on Phoenix, Nakai, ErlyDTL,
